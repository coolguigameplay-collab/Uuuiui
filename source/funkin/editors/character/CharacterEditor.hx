...
class CharacterEditor extends UIState {
    ...
    public override function create() {
        ...
        if (controls.mobileC)
            characterAnimsWindow.disableMouseControl = true; 
        ...
        if (controls.mobileC) buildMobileControls();
    }
    ...
    public override function update(elapsed:Float) {
        ...
        if (cameraHoverDummy.hovered && !draggingCharacter) {
            if (FlxG.mouse.wheel != 0 && !controls.mobileC || controls.mobileC && ScreenUtil.touch.wheel != 0) {
    ...
    var mobileExtraMode:Bool = false;

    inline function buildMobileControls() {
        var w:Dynamic = 88;
        var h:Dynamic = 88;
        var s:Dynamic = 10;
        var sw = FlxG.width;
		...
		var btnDown = new UIButton(btnX, startBtnY, "v", () -> { mobileExtraMode ? _offsets_extra_down(null) : _offsets_down(null); }, w, h);
		var btnUp = new UIButton(btnX, startBtnY + btnY, "^", () -> { mobileExtraMode ? _offsets_extra_up(null) : _offsets_up(null); }, w, h);
		btnX = s + w + s - (s / 2);
        var btnRight = new UIButton(btnX * 2, startBtnY, ">", () -> { mobileExtraMode ? _offsets_extra_right(null) : _offsets_right(null); }, w, h);

		var btnUndo = new UIButton(btnX * 5.6, startBtnY, "UNDO", () -> { _edit_undo(null); }, w, h);
		var btnRedo = new UIButton(btnX * 6.6, startBtnY, "REDO", () -> { _edit_redo(null); }, w, h);
        var btnCopy = new UIButton(btnX * 5.6, startBtnY + btnY, "COPY", () -> { _edit_copy_offset(null); }, w, h);
        var btnPaste = new UIButton(btnX * 6.6, startBtnY + btnY, "PASTE", () -> { _edit_paste_offset(null); }, w, h);

		var btnAnimUp = new UIButton(btnX * 6.6, startBtnY + (btnY * 2), "UP", () -> { _animation_up(null); }, w, h);
        var btnAnimDown = new UIButton(btnX * 6.6, startBtnY + (btnY * 3), "DOWN", () -> { _animation_down(null); }, w, h);
        var btnAnimPlay = new UIButton(btnX * 2, startBtnY + btnY, "ANIM\nPLAY", () -> { _animation_play(null); }, w, h);
        var btnZoomIn = new UIButton(btnX * 5.6, startBtnY + (btnY * 2), "Z+", () -> { _view_zoomin(null); }, w, h);
        var btnZoomOut = new UIButton(btnX * 5.6, startBtnY + (btnY * 3), "Z-", () -> { _view_zoomout(null); }, w, h);
		...
		*/
        ...
        var btns = [btnRight, btnDown, btnLeft, btnUp, btnExtra, btnUndo, btnRedo, btnCopy, btnPaste, btnAnimUp, btnAnimDown, btnAnimPlay, btnZoomIn, btnZoomOut];

        for (btn in btns) {
            btn.cameras = [uiCamera];
            btn.alpha = Options.controlsAlpha;
            btn.field.scale.set(1.5, 1.5);
